...
function NoteSales(props) {
  const noteSales = props.getValues("note_sales");
  ...
  const handleSubmit = () => {
    props.onCreate(noteDescr, 1);
    setNoteDescr("");
  };

  if (noteSales.length > 0) {
    const latestNote = noteSales.reduce((latest, current) => {
      ...
      <Grid container spacing={2}>
        ...
        <Grid size={12}>
          ...
          >
            <Grid size={1}></Grid>
            <Grid size={10}>
              <StyledLabel sx={{ textTransform: 'uppercase', height: 40, }}>note sales</StyledLabel>
            </Grid>
            <Grid size={1} sx={{ textAlign: 'end' }}>
              <IconButton
                onClick={() => props.onOpen(1)}
                sx={(theme) => ({
                  color: theme.vars.palette.grey.dark,
                })}
              >

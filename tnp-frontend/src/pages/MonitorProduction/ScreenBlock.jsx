...
function ScreenBlock({ data }) {
  ...
  const saveDate = data.screen_date
    ? moment(data.screen_date).format("DD/MM/yy")
    : null;
        ...
        <div className="content-date w-100 text-end rounded-end">
          {data.screen === 1 && (
            <>
              ...
              <label className="title ps-1 pe-3">{data.screen_block !== null && ` | ${data.screen_block}`}</label>
            </>
          )}
          {user.role !== "graphic" ||
          data.status === 2 ||
          data.screen === null ? null : (
            <>
              <input
                type="checkbox"
                className="btn-check"

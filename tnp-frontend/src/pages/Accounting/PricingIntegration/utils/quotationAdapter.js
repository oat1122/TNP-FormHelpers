...
export function adaptQuotationPayloadToPreview({ formData = {}, record = null, quotationNumber = '' } = {}) {
  ...
  const customer = {
    cus_company: src.customer?.cus_company || src.customer?.company || src.cus_company || src.company || '',
    cus_name: src.customer?.cus_name || src.customer?.name || src.cus_name || src.name || '',
    cus_address: src.customer?.cus_address || src.customer?.address || src.cus_address || src.address || '',
...
function sumItems(items) {
  return items.reduce((sum, it) => {
    ...
    if (rows.length > 0) {
      return sum + rows.reduce((s, r) => s + Number(r.quantity || 0) * Number(r.unit_price || 0), 0);
    }
    return sum + unit * qty;
  }, 0);
}

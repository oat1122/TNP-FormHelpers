...
export const useFormValidation = (initialFormData = {}) => {
  ...
  const validateField = useCallback((fieldName, value, step = null) => {
    ...
    switch (fieldName) {
      ...
      case 'title':
        if (!value?.trim()) {
          ...
          newErrors[fieldName] = `ชื่อเรื่องต้องไม่เกิน ${FORM_VALIDATION.MAX_TITLE_LENGTH} ตัวอักษร`;
        } else {
          delete newErrors[fieldName];
        }
        break;
      ...
      case 'notes':
        if (value && value.length > FORM_VALIDATION.MAX_NOTES_LENGTH) {
          newErrors[fieldName] = `หมายเหตุต้องไม่เกิน ${FORM_VALIDATION.MAX_NOTES_LENGTH} ตัวอักษร`;
        } else {
          delete newErrors[fieldName];
        }
        break;
      ...
      case 'special_instructions':
        if (value && value.length > FORM_VALIDATION.MAX_NOTES_LENGTH) {
          newErrors[fieldName] = `คำแนะนำพิเศษต้องไม่เกิน ${FORM_VALIDATION.MAX_NOTES_LENGTH} ตัวอักษร`;
        } else {
          delete newErrors[fieldName];
        }
        break;

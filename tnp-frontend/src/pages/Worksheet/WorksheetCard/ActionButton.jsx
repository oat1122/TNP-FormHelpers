...
import { setItem, setIsDuplicate, resetInputList } from "../../../features/Worksheet/worksheetSlice";
import { useDelWorksheetMutation, useUpdateWorksheetStatusMutation } from "../../../features/Worksheet/worksheetApi";
...
function ActionButton({ data, isSuccess, handleGenPdf }) {
  ...
  const handleDeleteWorksheet = async () => {
    const confirm = await dialog_delete_by_id(`Are you sure you want to delete the '${data.work_name}'?`);
  ...
  const handleConfirm = async (action) => {
    ...
    const confirm = await dialog_confirm_yes_no(`Do you want to ${action} the '${data.work_name}' worksheet ?`);
  ...
  const handleDuplicate = async () => {
    ...
    const confirm = await dialog_confirm_yes_no(`Do you want to copy the '${data.work_name}' worksheet ?`);
  ...
  if ((isOwner && user.role === "sale") || user.role === "admin") {
    ...
    if (data.status.code === 1 || user.role === "admin") {
      ...
      contentConfirmBtn = 
        <IconButton aria-label="confirm-worksheet" disabled={handleDisableConfirmBtn()} onClick={() => handleConfirm('confirm')}>
          <MdTaskAlt />
        </IconButton>
      ...
      >
        <Grid size={{ xl: 8 }}>
          {(user.role === "graphic" || user.role === "admin") 
            && (data.status.code !== 3 && data.status.code !== 4) ? (
            <IconButton
              aria-label="assign"
        ...
        <Grid size={{ xl: 4 }} textAlign="end">
          <IconButton
            ...
            onClick={() => handleGenPdf("work_sheet")}
          >
            <BsFileEarmarkPdf />
          </IconButton>
        </Grid>

...
function FabricReceived({ data }) {
  ...
  const orderDate = moment(data.received_start).format("DD/MM/yy");
  const receiveDate = moment(data.received_end).format("DD/MM/yy");
  ...
  const handleDisabledNoteButton = () => {
    return (
      item_lists.every((item) => item.pd_id !== data.pd_id || item.note_category !== "received") ||
      item_lists.length === 0
    );
  };
  ...
  const SettingRow = () => {
    ...
    const [orderDatePick, setOrderDatePick] = useState(moment(data.received_start));
    const [receiveDatePick, setReceiveDatePick] = useState(moment(data.received_end));
      ...
      <>
          ...
          >
            <form onSubmit={handleSubmitOrder}>
              <Modal.Header className="py-1">
                <Modal.Title className="mx-auto">Received Start Date</Modal.Title>
              </Modal.Header>
              <Modal.Body>
        ...
        <div className="content-setting border-end w-100">
          ...
          >
            <form onSubmit={handleSubmitReceive}>
              <Modal.Header className="py-1">
                <Modal.Title className="mx-auto">Received Finished Date</Modal.Title>
              </Modal.Header>
              <Modal.Body>
                <LocalizationProvider dateAdapter={AdapterMoment}>
                  <StaticDatePicker
        ...
        <div className="content-setting border-end w-100">
          ...
          <Modal show={showNote} onHide={handleNoteClose} size="lg" className="mt-5 modal-note">
            <ProductionNote pd_id={data.pd_id} category="received" />
          </Modal>
        </div>
        ...
        <div className="content-setting rounded-end">
          <input
            type="checkbox"
            className="btn-check"
            id={`btn-check-received-${data.pd_id}`}
            autoComplete="off"
            name="showTab"
            onChange={handleShow}
          />
          <label
            ...
            htmlFor={`btn-check-received-${data.pd_id}`}
          >
        ...
        {showTab ? (
          ...
          <>
            <div className="content-date text-start rounded-start ms-2 ps-2 w-25">
              <label className="title">ผ้าเข้า</label>
            </div>

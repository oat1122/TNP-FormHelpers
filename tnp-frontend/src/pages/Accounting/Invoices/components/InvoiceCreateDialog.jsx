...
const InvoiceCreateDialog = ({ open, onClose, quotationId, onCreated, onCancel }) => {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const dueDateForSave = isCredit
        ? selectedDueDate
          ? toISODate(selectedDueDate)
          : null
        : null;
      ...
      const payload = {
        ...
        document_header_type:
          documentHeaderType === "อื่นๆ" ? customHeaderType : documentHeaderType,

        // Financial calculations from frontend
        subtotal,
        special_discount_percentage:
          specialDiscountType === "percentage" ? specialDiscountValue : 0,
        special_discount_amount:
          specialDiscountType === "amount" ? specialDiscountValue : discountAmountComputed,
            ...
            <Stack spacing={3} sx={{ p: 1 }}>
                ...
                <Grid container spacing={3}>
                  <Grid item xs={12} md={6}>
                    <Box sx={{ p: 2, bgcolor: "grey.50", borderRadius: 2 }}>
                      ...
                      <Typography variant="subtitle1" fontWeight={700} className="long-text">
                        {customer?.cus_company || "-"}
                      </Typography>
                      <Typography variant="body2" color="text.secondary" className="long-text">
                  ...
                  <Grid item xs={12} md={6}>
                    <Box sx={{ p: 2, bgcolor: "grey.50", borderRadius: 2 }}>
                      ...
                      {isEditingAddress ? (
                        ...
                        <Typography variant="body2" fontWeight={500} className="long-text">
                          {customer?.cus_address || "-"}
                        </Typography>
                      )}
                    </Box>
              ...
              <Accordion defaultExpanded>
                ...
                <AccordionDetails>
                  <Stack spacing={3}>
                    {groups.map((g, gi) => {
                      const totalQty = (g.sizeRows || []).reduce(

      ...
      <CardContent>
                ...
                <li {...props} key={option.id}>
                  <Box>
                    ...
                    <Typography variant="body2" color="text.secondary">
                      ...
                      {option.due_date &&
                        ` | ครบกำหนด: ${dayjs(option.due_date).format("DD/MM/YYYY")}`}
                    </Typography>
                    ...
                    {(option.pattern_sizes || option.total_quantity) && (
                      <Typography variant="body2" color="info.main" sx={{ fontSize: "0.75rem" }}>
                        ...
                        {(() => {
                          ...
                          // Handle array format
                          if (
                            Array.isArray(option.pattern_sizes) &&
                            option.pattern_sizes.length > 0
                          ) {
                            const sizeWithQuantity = option.pattern_sizes
                              .map((s) => {
                                const sizeName = (s.size_name || s.size || s.name || "")
                                  .toString()
                                  .toUpperCase();
                          ...
                          if (
                            typeof option.pattern_sizes === "object" &&
                            !Array.isArray(option.pattern_sizes)
                          ) {
                            const genderSizes = [];

                            if (
                              option.pattern_sizes.men &&
                              Array.isArray(option.pattern_sizes.men) &&
                              option.pattern_sizes.men.length > 0
                            ) {
                            ...
                            if (
                              option.pattern_sizes.women &&
                              Array.isArray(option.pattern_sizes.women) &&
                              option.pattern_sizes.women.length > 0
                            ) {
                          ...
                          return (
                            sizeDisplay ||
                            (option.total_quantity ? `${option.total_quantity} ตัว` : "ไม่ระบุ")
                          );
                        })()}
                      </Typography>
                    )}
        ...
        <Alert severity="info" sx={{ mt: 2 }}>
          <Typography variant="body2">
            <strong>1.</strong> คลิกปุ่มรีเฟรช <Refresh fontSize="small" />{" "}
            เพื่อดึงข้อมูลล่าสุดจากระบบ NewWorkSheet
            <br />
            <strong>2.</strong> เลือกรายการที่ต้องการจากรายการ NewWorkSheet ด้านบน
            <br />
            <strong>3.</strong> ระบบจะกรอกข้อมูลให้อัตโนมัติตามข้อมูลที่มีใน NewWorkSheet
            <br />
          </Typography>
        </Alert>
      </CardContent>

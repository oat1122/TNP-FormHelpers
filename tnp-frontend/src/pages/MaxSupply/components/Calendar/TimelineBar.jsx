...
const TimelineBar = ({
  ...
  const baseTimelineOffset = isMobile
    ? CALENDAR_CONFIG.MOBILE_TIMELINE_OFFSET
    : CALENDAR_CONFIG.DESKTOP_TIMELINE_OFFSET;
  const timelineRowSpacing = isMobile
    ? CALENDAR_CONFIG.MOBILE_TIMELINE_SPACING
    : CALENDAR_CONFIG.DESKTOP_TIMELINE_SPACING;
  ...
  if (startWeek === endWeek) {
    ...
    const top =
      startWeek * calendarRowHeight + (calendarRowHeight - timelineRowSpacing * (rowIndex + 1));
    ...
    for (let week = startWeek + 1; week < endWeek; week++) {
      // ขยับให้ไปอยู่ล่างสุดของ cell แต่อยู่ภายใน cell อย่างปลอดภัย
      const middleWeekTop =
        week * calendarRowHeight + (calendarRowHeight - timelineRowSpacing * (rowIndex + 1));
      segments.push({
        left: "0%",
        width: "100%",
    ...
    if (endWeek > startWeek) {
      ...
      // ขยับให้ไปอยู่ล่างสุดของ cell แต่อยู่ภายใน cell อย่างปลอดภัย
      const lastWeekTop =
        endWeek * calendarRowHeight + (calendarRowHeight - timelineRowSpacing * (rowIndex + 1));
        ...
        >
          <Box
            ...
            sx={{
              ...
              width: segment.width,
              height: isMobile
                ? CALENDAR_CONFIG.MOBILE_TIMELINE_HEIGHT
                : CALENDAR_CONFIG.DESKTOP_TIMELINE_HEIGHT,
              top: segment.top,
              cursor: "pointer",
          ...
          >
            ...
            >
              ...
              {segment.isLastSegment &&
                timeline.duration > 1 &&
                !isMobile &&
                parseFloat(segment.width) > 15 && (
                  <Chip
                    label={
                      timeline.duration > 7
                        ? `${Math.ceil(timeline.duration / 7)}w`
                        : `${timeline.duration}d`
                    }
                    size="small"
                    sx={{

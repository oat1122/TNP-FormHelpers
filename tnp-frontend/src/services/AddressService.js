...
export class AddressService {
  ...
  static formatDisplayAddress(customer) {
    try {
      ...
      const isBangkok =
        customer.cus_province_name && customer.cus_province_name.includes("กรุงเทพ");

      // ชื่อสถานที่ - ใช้รูปแบบที่ถูกต้องตามจังหวัด
      if (customer.cus_subdistrict_name) {
        if (isBangkok) {
          parts.push(`แขวง${customer.cus_subdistrict_name}`);
        } else {
          parts.push(`ตำบล${customer.cus_subdistrict_name}`);
        }
      }
      ...
      if (customer.cus_district_name) {
        if (isBangkok) {
          ...
          parts.push(`อำเภอ${customer.cus_district_name}`);
        }
      }
    ...
    try {
      ...
      for (let i = 0; i < parts.length; i++) {
        if (parts[i].startsWith("จ.")) {
          ...
          parts.splice(i, 1);
          break;
        } else if (parts[i].startsWith("กรุงเทพฯ")) {
      ...
      // ตรวจสอบว่าเป็นกรุงเทพฯ หรือไม่
      const isBangkok = result.province === "กรุงเทพมหานคร";
      ...
      for (let i = 0; i < parts.length; i++) {
        if (parts[i].startsWith("เขต")) {
          ...
          parts.splice(i, 1);
          break;
        } else if (parts[i].startsWith("อำเภอ")) {
          result.district = parts[i].replace("อำเภอ", "");
          parts.splice(i, 1);
          break;
      ...
      for (let i = 0; i < parts.length; i++) {
        if (parts[i].startsWith("แขวง")) {
          ...
          parts.splice(i, 1);
          break;
        } else if (parts[i].startsWith("ตำบล")) {
          result.subdistrict = parts[i].replace("ตำบล", "");
          parts.splice(i, 1);
          break;
  ...
  static validateAddress(customer) {
    try {
      ...
      const hasComponents =
        customer.cus_province_name || customer.cus_district_name || customer.cus_subdistrict_name;

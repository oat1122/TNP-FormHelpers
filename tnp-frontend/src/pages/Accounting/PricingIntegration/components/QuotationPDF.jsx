...
const styles = StyleSheet.create({
  ...
  cell: { padding: 6, borderBottomWidth: 0.5, borderBottomColor: '#dddddd', borderBottomStyle: 'solid' },
  ...
  signLine: { borderBottomWidth: 0.75, borderColor: '#888', width: '80%', height: 24, marginBottom: 4 },
});
...
const Cell = ({ style, children, width }) => {
  const normalize = (c) => {
    ...
    if (Array.isArray(c)) return c.map((x, i) => <React.Fragment key={i}>{normalize(x)}</React.Fragment>);
...
const currencyTHB = (v) => new Intl.NumberFormat('th-TH', { style: 'currency', currency: 'THB' }).format(Number(v || 0));
const dateTH = (d) => {
  ...
  return (
    <Document title={data.quotationNumber ? `Quotation_${data.quotationNumber}` : 'Quotation'}>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.header}>
          <View style={styles.companyBox}>
            ...
            <View style={styles.companyInfo}>
              <Text style={{ fontSize: 14, fontWeight: 'bold' }}>{company.name || 'บริษัทของคุณ'}</Text>
              {company.address ? <Text style={styles.label}>{company.address}</Text> : null}
              <Text style={styles.label}>
                โทร: {company.phone || '-'}{company.taxId ? ` • เลขประจำตัวผู้เสียภาษี ${company.taxId}` : ''}
              </Text>
            </View>
          </View>
          <View style={styles.titleBox}>
            <Text style={styles.title}>ใบเสนอราคา</Text>
            {data?.quotationNumber ? <Text style={styles.label}>เลขที่: {data.quotationNumber}</Text> : null}
        ...
        <View style={{ marginTop: 8 }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
            <View style={{ width: '60%' }}>
              ...
              <Text style={{ fontWeight: 'bold' }}>{customer.cus_company || customer.company || '-'}</Text>
              {customer.cus_name || customer.name ? <Text>{customer.cus_name || customer.name}</Text> : null}
              {customer.cus_address || customer.address ? <Text>{customer.cus_address || customer.address}</Text> : null}
              {customer.cus_phone || customer.phone ? <Text>โทร: {customer.cus_phone || customer.phone}</Text> : null}
            </View>
            <View style={{ width: '35%' }} />
          </View>
        </View>
        ...
        {/* Table header */}
        <View style={[styles.table, { borderTopWidth: 1, borderTopColor: '#e0e0e0', borderTopStyle: 'solid' }]}>
          <View style={[styles.row, styles.th]}>
            <Cell width={30}>#</Cell>
            <Cell width={260}>รายละเอียดงาน</Cell>
            <Cell width={80} style={styles.right}>จำนวน</Cell>
            <Cell width={90} style={styles.right}>ราคาต่อหน่วย</Cell>
            <Cell width={100} style={styles.right}>ยอดรวม</Cell>
          </View>

          {/* Table Rows */}
          {items.length === 0 ? (
            <View style={styles.row}><Cell width={560}>ไม่มีรายการ</Cell></View>
          ) : (
            items.map((it, idx) => {
              ...
              return (
                <View key={it.id || idx} wrap={false}>
                  ...
                  <View style={styles.row}>
                    ...
                    <Cell width={260}><Text style={{ fontWeight: 'bold' }}>{it.name || 'ไม่ระบุชื่องาน'}</Text></Cell>
                    <Cell width={80} />
                    <Cell width={90} />
                    <Cell width={100} />
                  </View>
                  ...
                  {rows.length > 0 ? (
                    rows.map((r, i) => {
                      ...
                      return (
                        <View key={`r-${idx}-${i}`} style={styles.row}>
                          ...
                          <Cell width={80} style={styles.right}><Text>{`${rQty.toLocaleString('th-TH')} ${unit}`}</Text></Cell>
                          <Cell width={90} style={styles.right}><Text>{currencyTHB(rUnit)}</Text></Cell>
                          <Cell width={100} style={styles.right}><Text>{currencyTHB(rTotal)}</Text></Cell>
                        </View>
                      );
                    })
                  ) : (
                    <View style={styles.row}>
                      <Cell width={30}></Cell>
                      <Cell width={260}>
                        {it.notes ? <Text style={{ color: '#666', fontStyle: 'italic' }}>หมายเหตุ: {it.notes}</Text> : <Text></Text>}
                      </Cell>
                      <Cell width={80} style={styles.right}><Text>{`${qty.toLocaleString('th-TH')} ${unit}`}</Text></Cell>
                      <Cell width={90} style={styles.right}><Text>{currencyTHB(unitPrice)}</Text></Cell>
                      <Cell width={100} style={styles.right}><Text>{currencyTHB(totalLine)}</Text></Cell>
                    </View>
                  )}
                  ...
                  <View style={styles.row}>
                    ...
                    <Cell width={430}>
                      {(rows.length > 0 && it.notes) ? (
                        <Text style={{ color: '#666', fontStyle: 'italic' }}>หมายเหตุ: {it.notes}</Text>
                      ) : <Text></Text>}
                    </Cell>
                    <Cell width={90}></Cell>
                    <Cell width={100} style={styles.right}><Text style={{ fontWeight: 'bold' }}>{currencyTHB(totalLine)}</Text></Cell>
                  </View>

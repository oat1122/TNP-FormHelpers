            ...
            return (
              <Box
                ...
                sx={{
                  ...
                  position: "relative",
                  backgroundColor: isTodayDate ? "#e8f0fe" : isCurrentMonth ? "#fff" : "#f9fafb",
                ...
                <Box sx={{ p: isMobile ? 0.5 : 1 }}>
                  ...
                  <Box sx={{ mt: 0.25, display: "flex", flexDirection: "column", gap: 0.2 }}>
                    {dayEvents.slice(0, 2).map((event) => {
                      ...
                      const typeConfig =
                        productionTypeConfig[event.production_type] || productionTypeConfig.screen;
                            ...
                            sx={{
                              ...
                              border:
                                event.priority === "urgent" || event.priority === "high"
                                  ? `2px solid ${priorityInfo.color}`
                                  : "none",
                              animation:
                                event.priority === "urgent" ? "pulse 1.5s infinite" : "none",
                              "@keyframes pulse": {
                                "0%, 100%": { opacity: 0.9 },
                                "50%": { opacity: 1 },
                              },
                    ...
                    {(() => {
                      const startingJobsCount = dayEvents.filter(
                        (event) =>
                  ...
                  {(() => {
                    const dayEventsTimelines = eventRows.reduce((acc, row, rowIndex) => {
                      const dayTimelines = row.filter((timeline) => {
                        const timelineStartDate = format(
                          parseISO(timeline.event.start_date),
                          "yyyy-MM-dd"
                        );
              ...
              {(eventRows.length > 5 || overflowTimelines.length > 0) && (
                ...
                >
                  <Typography variant="body2" fontWeight="bold">
                    +
                    {(eventRows.length > 5 ? eventRows.slice(5).flat().length : 0) +
                      overflowTimelines.length}{" "}
                    งานเพิ่มเติม
                  </Typography>

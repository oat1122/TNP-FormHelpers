...
class QuotationPdfService
{
    ...
    public function render(Quotation $q): string
    {
        ...
        try {
            $this->pdf->SetFont($boldFont, '', 18);
        } catch (\Throwable $e) {
            $this->pdf->SetFont($fallbackBold[0], $fallbackBold[1], 18);
        }
        $this->pdf->Cell(78, 8, $this->t('ใบเสนอราคา'), 0, 2, 'R');
        try {
            $this->pdf->SetFont($regularFont, '', 12);
        } catch (\Throwable $e) {
            $this->pdf->SetFont($fallbackRegular[0], $fallbackRegular[1], 12);
        }
        ...
        try {
            $this->pdf->SetFont($boldFont, '', 16);
        } catch (\Throwable $e) {
            $this->pdf->SetFont($fallbackBold[0], $fallbackBold[1], 16);
        }
        ...
        $maxLabelW = 0;
        foreach ($labels as $L) {
            $w = $this->pdf->GetStringWidth($this->t($L));
            if ($w > $maxLabelW) {
                $maxLabelW = $w;
            }
        }
        ...
        try {
            $this->pdf->SetFont($boldFont, '', 12);
        } catch (\Throwable $e) {
            $this->pdf->SetFont($fallbackBold[0], $fallbackBold[1], 12);
        }
        ...
        foreach (array_values($groups) as $g) {
            ...
            if (! empty($descParts)) {
                try {
                    $this->pdf->SetFont($regularFont, '', 11);
                } catch (\Throwable $e) {
                    $this->pdf->SetFont($fallbackRegular[0], $fallbackRegular[1], 11);
                }

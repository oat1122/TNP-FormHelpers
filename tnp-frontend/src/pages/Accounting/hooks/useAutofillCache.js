...
export const useAutofillCache = (prIds = [], options = {}) => {
  ...
  const getAutofillForPR = useCallback(
    (prId) => {
...
export const useAutofillCacheManager = () => {
  ...
  const invalidateAutofill = useCallback(
    (prIds) => {
      if (!Array.isArray(prIds)) prIds = [prIds];

      prIds.forEach((prId) => {
        dispatch(accountingApi.util.invalidateTags([{ type: "PricingRequest", id: prId }]));
      });
    },
    [dispatch]
  );
  ...
  const updateAutofillCache = useCallback(
    (prId, data) => {
      dispatch(
        accountingApi.util.upsertQueryData("getPricingRequestAutofill", prId, () => ({
          success: true,
          data,
        }))
      );

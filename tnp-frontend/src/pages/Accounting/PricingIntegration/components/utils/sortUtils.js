...
export const sortPricingRequestsByLatest = (requests = []) => {
  ...
  return arr.sort((a, b) => {
    const aCreated = parseDate(a?.created_at) || parseDate(a?.pr_created_date) || parseDate(a?.updated_at);
    const bCreated = parseDate(b?.created_at) || parseDate(b?.pr_created_date) || parseDate(b?.updated_at);

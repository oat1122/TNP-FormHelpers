...
function ExamOrder({ data }) {
  ...
  const orderDate = moment(data.exam_start).format("DD/MM/yy");
  const receiveDate = moment(data.exam_end).format("DD/MM/yy");
  ...
  const handleDisabledNoteButton = () => {
    return (
      item_lists.every((item) => item.pd_id !== data.pd_id || item.note_category !== "example") ||
      item_lists.length === 0
    );
  };
  ...
  const SettingRow = () => {
    ...
    const [orderDatePick, setOrderDatePick] = useState(moment(data.exam_start));
    const [receiveDatePick, setReceiveDatePick] = useState(moment(data.exam_end));
    ...
    const handleSubmitOrder = async (event) => {
      ...
      try {
        ...
        if (response.data.success) {
          ...
          // แสดง toast แทน Swal
          await open_dialog_ok_timer("Example date updated");
      ...
      <>
          ...
          >
            <form onSubmit={handleSubmitOrder}>
              <Modal.Header className="py-1">
                <Modal.Title className="mx-auto">Example Start Date</Modal.Title>
              </Modal.Header>
              <Modal.Body>
        ...
        <div className="content-setting border-end w-100">
          ...
          >
            <form onSubmit={handleSubmitReceive}>
              <Modal.Header className="py-1">
                <Modal.Title className="mx-auto">Example Finished Date</Modal.Title>
              </Modal.Header>
              <Modal.Body>
                <LocalizationProvider dateAdapter={AdapterMoment}>
                  <StaticDatePicker
        ...
        <div className="content-setting border-end w-100">
          ...
          <Modal show={showNote} onHide={handleNoteClose} size="lg" className="mt-5 modal-note">
            <ProductionNote pd_id={data.pd_id} category="example" />
          </Modal>
        </div>
        ...
        <div className="content-setting rounded-end">
          ...
          <label
            ...
            htmlFor={`btn-check-exam-${data.pd_id}`}
          >
        ...
        {showTab ? (
          ...
          <>
            <div className="content-date text-start rounded-start ms-2 ps-2 w-25">
              <label className="title">ตัวอย่าง</label>
            </div>

...
import {
  ...
  Box, Stack, TextField, Button, IconButton, Typography,
    ...
    <Dialog open={open} onClose={handleClose} maxWidth="md" fullWidth>
      ...
      <DialogContent dividers>
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 2 }}>
          <Box>
            <Typography variant="subtitle2" gutterBottom>ฟอร์ม</Typography>
            <Stack spacing={1.25}>
              <TextField size="small" label="ชื่อ (ย่อ/แสดง)" value={form.name} onChange={(e) => setForm(f => ({ ...f, name: e.target.value }))} fullWidth />
              <TextField size="small" label="ชื่อจดทะเบียน" value={form.legal_name} onChange={(e) => setForm(f => ({ ...f, legal_name: e.target.value }))} fullWidth />
              <TextField size="small" label="สาขา" value={form.branch} onChange={(e) => setForm(f => ({ ...f, branch: e.target.value }))} fullWidth />
              <TextField size="small" label="ที่อยู่" value={form.address} onChange={(e) => setForm(f => ({ ...f, address: e.target.value }))} fullWidth multiline minRows={2} />
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.25}>
                <TextField size="small" label="เลขผู้เสียภาษี" value={form.tax_id} onChange={(e) => setForm(f => ({ ...f, tax_id: e.target.value }))} fullWidth />
                <TextField size="small" label="โทรศัพท์" value={form.phone} onChange={(e) => setForm(f => ({ ...f, phone: e.target.value }))} fullWidth />
              </Stack>
          ...
          <Box>
            <Typography variant="subtitle2" gutterBottom>รายการบริษัท</Typography>
            <List dense sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 1 }}>
              {(companies || []).map((c) => (
                <ListItem key={c.id} divider>
                  <ListItemText
                    primary={<Typography variant="body2" sx={{ fontWeight: 600 }}>{c.short_code || c.name}</Typography>}
                    secondary={
                      <Typography variant="caption" color="text.secondary">
                        {c.name}{c.is_active === false ? ' • ปิดใช้งาน' : ''}
                      </Typography>
                    }
                  />
              ...
              {(!companies || companies.length === 0) && (
                <Box sx={{ p: 2, textAlign: 'center' }}><Typography variant="body2" color="text.secondary">ไม่มีข้อมูลบริษัท</Typography></Box>
              )}
            </List>
          </Box>
        </Box>

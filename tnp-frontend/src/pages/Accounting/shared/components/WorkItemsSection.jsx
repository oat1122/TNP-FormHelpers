 ...
 */
const WorkItemsSection = ({ items = [], title = "รายละเอียดงาน", icon, children }) => {
  return (
    <Section>
      <SectionHeader>
        {icon && (
      ...
      <Box sx={{ p: 2 }}>
        ...
        {items.length === 0 ? (
          ...
          children ||
          items.map((item, idx) => <WorkItemCard key={item.id || idx} item={item} index={idx} />)
        )}
      </Box>
    </Section>
  );

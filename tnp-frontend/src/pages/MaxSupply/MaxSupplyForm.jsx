...
const MaxSupplyForm = () => {
  ...
  const processWorksheetData = (data) => {
    ...
    try {
      ...
      const validWorksheets = worksheetItems.filter(
        (ws) => ws && (ws.worksheet_id || ws.id || ws.work_id)
      );
      ...
      console.log(
        "Worksheets after production filter (has_production !== true):",
      ...
      if (productionWorksheets.length > 0) {
        console.log(
          "Worksheets filtered out (has_production === true):",
          productionWorksheets.slice(0, 5)
        );
      }

      return filteredWorksheets.map((ws, index) => {
  ...
  const parsePrintLocations = (worksheet) => {
    ...
    if (worksheet.screen_detail) {
      ...
      if (
        (screenDetail.includes("ปัก") || screenDetail.includes("embroider")) &&
        worksheet.screen_embroider
      ) {
    ...
    if (
      !printLocations.screen.enabled &&
      worksheet.screen_point &&
      parseInt(worksheet.screen_point) > 0
    ) {
      printLocations.screen.enabled = true;
    ...
    if (
      !printLocations.embroidery.enabled &&
      worksheet.screen_embroider &&
      parseInt(worksheet.screen_embroider) > 0
    ) {
      printLocations.embroidery.enabled = true;
    ...
    if (
      !printLocations.sublimation.enabled &&
      worksheet.screen_flex &&
      parseInt(worksheet.screen_flex) > 0
    ) {
      printLocations.sublimation.enabled = true;
      printLocations.sublimation.points = parseInt(worksheet.screen_flex);
      printLocations.sublimation.position = "ไม่ระบุ";
      console.log(
        "Flex/Sublimation fallback enabled with points:",
        printLocations.sublimation.points
      );
    }
  ...
  const parseSizeBreakdown = (worksheet) => {
    ...
    try {
        ...
        if (Array.isArray(worksheet.pattern_sizes)) {
          ...
          worksheet.pattern_sizes.forEach((sizeInfo) => {
            ...
            if (sizeInfo.quantity && parseInt(sizeInfo.quantity) > 0) {
              ...
              quantity = Math.floor(
                parseInt(worksheet.total_quantity) / worksheet.pattern_sizes.length
              );
            }
          ...
          if (worksheet.pattern_sizes.men && Array.isArray(worksheet.pattern_sizes.men)) {
            worksheet.pattern_sizes.men.forEach((sizeInfo) => {
              ...
              if (sizeInfo.quantity && parseInt(sizeInfo.quantity) > 0) {
                ...
                quantity = Math.floor(
                  parseInt(worksheet.total_quantity) / worksheet.pattern_sizes.men.length
                );
              }
          ...
          if (worksheet.pattern_sizes.women && Array.isArray(worksheet.pattern_sizes.women)) {
            worksheet.pattern_sizes.women.forEach((sizeInfo) => {
              ...
              if (sizeInfo.quantity && parseInt(sizeInfo.quantity) > 0) {
                ...
                quantity = Math.floor(
                  parseInt(worksheet.total_quantity) / worksheet.pattern_sizes.women.length
                );
              }
      ...
      // Try worksheet.sizes as array
        ...
        worksheet.sizes.forEach((size) => {
          ...
          const quantity = worksheet.size_quantities
            ...
            : worksheet.quantities && worksheet.quantities[size]
              ? worksheet.quantities[size]
              : 0;
          sizeBreakdown.push({ size: size.toUpperCase(), quantity: parseInt(quantity) });
        });
      }
      // Look for size_S, size_M, size_L pattern in the data
        ...
        const sizeKeys = Object.keys(worksheet).filter(
          (key) => key.startsWith("size_") || key.match(/^[smlx]{1,4}$/i)
        );
        if (sizeKeys.length > 0) {
          sizeKeys.forEach((key) => {
            const sizeName = key.startsWith("size_")
              ? key.replace("size_", "").toUpperCase()
              : key.toUpperCase();
      ...
      if (sizes.length === 0 && worksheet.total_quantity > 0) {
        console.log(
          "No sizes found, creating default distribution from total_quantity:",
          worksheet.total_quantity
        );
  ...
  const handleWorksheetSelect = (worksheet) => {
    ...
    if (worksheet) {
      ...
      const totalQuantity =
        sizeBreakdown.reduce((sum, item) => sum + item.quantity, 0) ||
        worksheet.total_quantity ||
        0;
      ...
      const mapShirtType = (typeShirt) => {
        ...
        if (type === "tank-top" || type.includes("tank") || type.includes("กล้าม"))
          return "tank-top";
        if (type === "long-sleeve" || type.includes("long") || type.includes("แขนยาว"))
          return "long-sleeve";
      ...
      const determineProductionType = () => {
        ...
        if (worksheet.screen_point && parseInt(worksheet.screen_point) > 0)
          enabledTypes.push("screen");
        if (worksheet.screen_embroider && parseInt(worksheet.screen_embroider) > 0)
          enabledTypes.push("embroidery");
        if (worksheet.screen_flex && parseInt(worksheet.screen_flex) > 0)
          enabledTypes.push("sublimation"); // Using sublimation for flex
      ...
      const autoFillData = {
        ...
        title:
          worksheet.product_name || worksheet.work_name || `${worksheet.customer_name} - งานใหม่`,
        ...
        expected_completion_date: worksheet.due_date
          ? dayjs(worksheet.due_date).subtract(2, "day")
          : dayjs().add(7, "day"),
        ...
        special_instructions:
          worksheet.worksheet_note ||
          worksheet.special_instructions ||
          worksheet.screen_detail ||
          "",
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log(
        "Is valid production_type:",
        ["screen", "dtf", "sublimation", "embroidery"].includes(submitData.production_type)
      );
      console.log("============================");
  ...
  const manualRefreshWorksheets = async () => {
    try {
      ...
      if (
        ...
        (response.data || response.worksheets)
      ) {
    ...
    <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale="th">
      <Box className="max-supply-form">
        ...
        {isSmallMobile && (
          ...
          >
            <LanguageToggle language={language} onLanguageChange={changeLanguage} compact={true} />
          </Box>
        )}
              ...
              {autoFillPreview && (
                ...
                >
                  <Typography variant="body2" className={language === "my" ? "myanmar-text" : ""}>
                    <strong>{t("autoFillMessage")}</strong>
                    <br />
                    {t("customerLabel")} {autoFillPreview.customer_name} |{t("quantityLabel")}{" "}
                    {autoFillPreview.total_quantity} {t("pieces")} |{t("typeLabel")}{" "}
                    {autoFillPreview.production_type} |{t("dueDateLabel")}{" "}
                    {autoFillPreview.due_date.format("DD/MM/YYYY")}
                    {autoFillPreview.newworks_code &&
                      ` | ${t("codeLabel")} ${autoFillPreview.newworks_code}`}
                    {autoFillPreview.fabric_info?.fabric_name &&
                      ` | ${t("fabricLabel")} ${autoFillPreview.fabric_info.fabric_name}`}
                  </Typography>
                </Alert>
              )}

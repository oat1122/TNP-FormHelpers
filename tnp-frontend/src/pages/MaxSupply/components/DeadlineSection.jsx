...
const DeadlineSection = ({
  ...
  const primaryData =
    jobs.length > 0 ? jobs : getUpcomingDeadlines ? getUpcomingDeadlines(daysAhead) : [];
  ...
  const defaultFilter = (items) => {
    ...
    return items.filter((job) => {
      ...
      try {
        const expectedDate =
          typeof targetDate === "string" ? parseISO(targetDate) : new Date(targetDate);
  ...
  const upcomingDeadlines = filteredDeadlines.sort((a, b) => {
    ...
    const dateA =
      typeof (a.expected_completion_date || a.due_date) === "string"
        ? parseISO(a.expected_completion_date || a.due_date)
        : new Date(a.expected_completion_date || a.due_date);
    const dateB =
      typeof (b.expected_completion_date || b.due_date) === "string"
                ...
                secondary={
                  item.expected_completion_date || item.due_date
                    ? (() => {
                        try {
                          const targetDate =
                            typeof (item.expected_completion_date || item.due_date) === "string"

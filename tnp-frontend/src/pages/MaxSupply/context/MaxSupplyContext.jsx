  ...
  switch (action.type) {
    ...
    case ActionTypes.UPDATE_MAX_SUPPLY:
      return {
        ...
        currentMaxSupply: state.currentMaxSupply?.id === action.payload.id 
          ? action.payload 
    ...
    case ActionTypes.DELETE_MAX_SUPPLY:
      return {
        ...
        currentMaxSupply: state.currentMaxSupply?.id === action.payload 
          ? null 
          : state.currentMaxSupply,
      };
...
export const MaxSupplyProvider = ({ children }) => {
    ...
    loadCalendarData: async (params = {}) => {
      try {
        ...
        if (response.status === 'success') {
          ...
          dispatch({ type: ActionTypes.SET_CALENDAR_STATISTICS, payload: response.data.statistics || {} });
        }
      } catch (error) {
        dispatch({ type: ActionTypes.SET_ERROR, payload: error.message });
      }
  ...
  return (
    <MaxSupplyContext.Provider value={{ state, actions }}>
      {children}

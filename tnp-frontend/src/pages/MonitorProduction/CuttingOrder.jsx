...
function CuttingOrder({ data }) {
  ...
  const orderDate = moment(data.cutting_start).format("DD/MM/yy");
  const receiveDate = moment(data.cutting_end).format("DD/MM/yy");
  ...
  const handleDisabledNoteButton = () => {
    return (
      item_lists.every((item) => item.pd_id !== data.pd_id || item.note_category !== "cutting") ||
      item_lists.length === 0
    );
  };
  ...
  const SettingRow = () => {
    ...
    const [orderDatePick, setOrderDatePick] = useState(moment(data.cutting_start));
    const [receiveDatePick, setReceiveDatePick] = useState(moment(data.cutting_end));
    ...
    const handleSubmitOrder = async (event) => {
      ...
      try {
        ...
        if (response.data.success) {
          ...
          // แสดง toast แทน Swal
          await open_dialog_ok_timer("Cutting date updated");
      ...
      <>
          ...
          >
            <form onSubmit={handleSubmitOrder}>
              <Modal.Header className="py-1">
                <Modal.Title className="mx-auto">Cutting Start Date</Modal.Title>
              </Modal.Header>
              <Modal.Body>
        ...
        <div className="content-setting border-end w-100">
          ...
          >
            <form onSubmit={handleSubmitReceive}>
              <Modal.Header className="py-1">
                <Modal.Title className="mx-auto">Cutting Finished Date</Modal.Title>
              </Modal.Header>
              <Modal.Body>
                <LocalizationProvider dateAdapter={AdapterMoment}>
                  <StaticDatePicker
        ...
        <div className="content-setting border-end w-100">
          ...
          <Modal show={showNote} onHide={handleNoteClose} size="lg" className="mt-5 modal-note">
            <ProductionNote pd_id={data.pd_id} category="cutting" />
          </Modal>
        </div>
        ...
        <div className="content-setting rounded-end">
          <input
            type="checkbox"
            className="btn-check"
            id={`btn-check-cutting-${data.pd_id}`}
            autoComplete="off"
            name="showTab"
            onChange={handleShow}
          />
          <label
            ...
            htmlFor={`btn-check-cutting-${data.pd_id}`}
          >

...
const ApprovalPanel = ({
    ...
    return (
      <Box>
        ...
        <Typography variant="body2" color="text.secondary">
          เลือกจากด้านซ้ายเพื่อทำการตรวจสอบ
        </Typography>
      </Box>
    );
  }
  ...
  return (
    <Box>
      ...
      <Typography variant="body2" color="text.secondary">
        {q?.number} • {q?.customer?.cus_company || q?.work_name || "-"}
      </Typography>
          ...
          <Paper variant="outlined" sx={{ p: 2 }}>
            ...
            <Stack spacing={0.5}>
              ...
              <Typography variant="body2">
                ผู้สร้าง: {q?.created_by_name || q?.created_by || "-"}
              </Typography>
            </Stack>
          </Paper>
        </Grid>
          ...
          <Paper variant="outlined" sx={{ p: 2 }}>
            ...
            <Stack spacing={0.5}>
              ...
              <Typography variant="body2" fontWeight={700}>
                รวมทั้งสิ้น: {formatTHB(q?.total_amount)}
              </Typography>
              <Typography variant="body2">
                มัดจำ: {q?.deposit_percentage ? `${q.deposit_percentage}%` : "0%"} (
                {formatTHB(q?.deposit_amount)})
              </Typography>
          ...
          <Paper variant="outlined" sx={{ p: 2 }}>
            <Stack direction="row" alignItems="center" justifyContent="space-between" mb={1}>
              ...
              <Stack direction="row" spacing={1}>
                <Chip
                  size="small"
                  label={`จำนวน ${Array.isArray(q?.items) ? q.items.length : 0} รายการ`}
                />
                {prIds.length > 0 && (
                  <Tooltip title="ดูงาน Pricing ที่อ้างอิง">
                    <Button
                      size="small"
                      variant="outlined"
                      onClick={() => onOpenLinkedPricing?.(q.id)}
                    >
      ...
      <Stack spacing={1} mt={2}>
        <Stack direction="row" spacing={1}>
          ...
          <Button
            ...
            onClick={() => onSendBack?.(q.id, reason || "กรุณาปรับรายละเอียดบางรายการ")}
          >
            ส่งกลับแก้ไข
          </Button>
          <Button
            ...
            color="error"
            disabled={!canReject}
            onClick={() => onReject?.(q.id, reason || "ไม่ผ่านเงื่อนไขเครดิต")}
          >
            ปฏิเสธ
          </Button>
        </Stack>
      ...
      <Stack direction="row" spacing={1}>
        <Button disabled={!canDownload} onClick={() => onDownloadPDF?.(q.id)}>
          ดาวน์โหลด PDF
        </Button>
        <Button
          disabled={q.status !== "approved"}
          onClick={() => onMarkSent?.(q.id, { delivery_method: "email" })}
        >
          บันทึกว่าจัดส่งแล้ว
        </Button>
      </Stack>
      ...
      <Stack direction="row" spacing={1} mt={1}>
        <Button
          ...
          variant="outlined"
          onClick={() => onUploadEvidence?.(q.id, files, desc)}
        >
          อัปโหลดหลักฐาน
        </Button>
      </Stack>

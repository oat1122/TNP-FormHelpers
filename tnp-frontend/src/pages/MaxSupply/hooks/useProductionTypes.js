...
export const useProductionTypes = () => {
  ...
  const getProductionTypeIcon = useCallback((type) => {
    const config = getProductionTypeConfig(type);
    return config.icon;
  }, [getProductionTypeConfig]);

  // Get production type color
  const getProductionTypeColor = useCallback((type) => {
    const config = getProductionTypeConfig(type);
    return config.color;
  }, [getProductionTypeConfig]);
  ...
  const getProductionTypesWithCapacity = useCallback((jobs) => {
    return Object.keys(productionTypeConfig).map(type => {
      ...
      return {
        ...
        completion_rate: capacity.total_jobs > 0 
  ...
  const getFilteredJobs = useCallback((jobs) => {
    ...
    return filtered;
  }, [selectedType, selectedStatus, selectedPriority, filterJobsByType, filterJobsByStatus, filterJobsByPriority]);

  // Get summary statistics
  const getSummaryStats = useCallback((jobs) => {
  ...
  const parsePrintLocations = useCallback((job) => {
  ...
  const getJobPriorityColor = useCallback((job) => {
    const config = getPriorityConfig(job.priority);
    return config.color;
  }, [getPriorityConfig]);

  // Get job status color
  const getJobStatusColor = useCallback((job) => {
  ...
  const isJobUrgent = useCallback((job) => {
    return job.priority === 'urgent' || isJobOverdue(job);
  }, [isJobOverdue]);

  // Get job display info
  const getJobDisplayInfo = useCallback((job) => {
  ...
  }, [getProductionTypeConfig, getStatusConfig, getPriorityConfig, isJobOverdue, isJobUrgent, parsePrintLocations]);

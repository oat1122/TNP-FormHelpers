...
const PRGroupCalcCard = React.memo(function PRGroupCalcCard({
  group,
  index,
  isEditing,
  onAddRow,
...
const QuotationDetailDialog = ({ open, onClose, quotationId }) => {
  ...
  const [uploadSignatures, { isLoading: isUploadingSignatures }] =
    useUploadQuotationSignaturesMutation();
  const [deleteSignatureImage, { isLoading: isDeletingSignature }] =
    useDeleteQuotationSignatureImageMutation();
    ...
    try {
      await updateQuotation({
        ...
        deposit_percentage:
          depositMode === "percentage"
            ? Number(depositPct || 0)
            : Number(liveDepositPercentage || 0),
                        ...
                        <Grid item xs={12} md={6}>
                          <SpecialDiscountField
                            ...
                            onDiscountTypeChange={(t) => {
                              if (!isEditing) return;
                              setSpecialDiscountType(t);
                            }}
                            onDiscountValueChange={(v) => {
                              if (!isEditing) return;
                              setSpecialDiscountValue(v);
                            }}
                            disabled={!isEditing}
                          />
                        </Grid>
                        <Grid item xs={12} md={6}>
                          <WithholdingTaxField
                            ...
                            onTaxPercentageChange={(p) =>
                              isEditing && setWithholdingTaxPercentage(p)
                            }
                            disabled={!isEditing}
                          />
                        </Grid>
                    ...
                    <Box sx={{ p: 2 }}>
                      <PaymentTerms
                        ...
                        onChangeDepositPercentage={(v) =>
                          isEditing && setDepositPct(sanitizeInt(v))
                        }
                        onChangeDepositAmount={(v) => isEditing && setDepositAmountInput(v)}
                        isCredit={
                          isEditing
                            ? paymentTermsType === "credit_30" || paymentTermsType === "credit_60"
                            : paymentMethod !== "cash"
                        }
                        dueDateNode={
                          (
                  ...
                  <Box sx={{ mt: 1 }}>
                    ...
                    <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1, mt: 1 }}>
                      {(sampleImages || []).map((img) => {
                        ...
                        const src = img.url || "";
                        const checked =
                          selectedSampleForPdfLocal !== null
            ...
            <SecondaryButton
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  await deleteSignatureImage({
                    id: q.id,
                    identifier: previewImage.filename || "",
                  }).unwrap();
                  dismissToast(loadingId);
                  showSuccess("ลบรูปสำเร็จ");
